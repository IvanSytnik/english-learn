...
Status: MVP-0.5 Week 2 in progress. Selection Policy (Thompson Sampling) COMPLETE + verified live. Node bumped 20 → 22 LTS.
Typecheck clean across 6 packages, 192/192 tests passing (learner-model 139 · db 44 · ui 3 · web 6 · ai 0), dev server runs locally.
Next: Diagnostic v2 (recommended — unblocks cold-start #4 + resolves #3, not externally blocked). Then AI Tutor context builder (blocked on AI provider strategy), eval harness.
...
8. 🔴 Открытые вопросы
#ВопросБлокируетПриоритет1AI provider strategy wiring (Claude + OpenAI fallback)AI-generation pipeline; нужен до AI Tutor context builder🟡 Medium2Listening audio source (TTS vs open-source datasets)Listening exercises + seed🟡 Medium3DiagnosticItem polymorphic vs 3 таблицыРасширение pool; решается в Diagnostic v2🟢 Low4Cold-start bootstrap ученика из diagnostic → ConceptMastery«умный» старт selection для новичка (сейчас работает informative IRT-prior, но diagnostic не кормит mastery)🟡 Medium → Diagnostic v2
...
Diagnostic v2 — заменяет pseudo-CAT; решает open question #3 (polymorphic); включает cold-start bootstrap (#4) — инициализация ConceptMastery из diagnostic-результатов. Не заблокирован ничем внешним → рекомендуемый следующий шаг. Clarifying questions намечены (ниже)
