...
describe('effectiveStrategy (calibration gate)', () => {
  it('bernoulli stays bernoulli regardless of count', () => {
    expect(effectiveStrategy(cfg({ strategy: 'bernoulli' }), 0)).toBe('bernoulli');
    expect(effectiveStrategy(cfg({ strategy: 'bernoulli' }), 10_000)).toBe('bernoulli');
  });
...
describe('rewardForOutcome', () => {
  ...
  it('learning_gain: clamped positive delta', () => {
    expect(rewardForOutcome(true, 0.4, 0.7, 'learning_gain', 0.5)).toBeCloseTo(0.3, 10);
  ...
  it('throws on NaN rather than masking it', () => {
    expect(() => rewardForOutcome(true, Number.NaN, 0.5, 'learning_gain', 0.5)).toThrow(/NaN/);
  });
});
