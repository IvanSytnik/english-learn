  ...
  it("returns NO_CANDIDATES when every candidate is introduction-locked", () => {
    const out = select(
      baseInput({
        ...
        prereqEdges: [
          { from: "past_simple", to: "present_perfect", kind: "PREREQUISITE" },
        ],
        masteries: [
          {
  ...
  it("never returns a locked concept", () => {
    fc.assert(
      fc.property(fc.integer(), (seed) => {
        ...
        const input = baseInput({
          candidates: [
            item("locked", "present_perfect", null),
            item("free", "vocab.free", null),
          ],
          prereqEdges: [
            {
              from: "past_simple",
...
describe("select — item choice within a concept", () => {
  it("prefers earliest-due seen item over unseen within the chosen concept", () => {
    ...
    const out = select(
      baseInput({
        candidates: [
          item("unseen", "c.only", null),
          item("seen-soon", "c.only", NOW + 10_000),
        ],
      }),
      makeRng(3),
    );
