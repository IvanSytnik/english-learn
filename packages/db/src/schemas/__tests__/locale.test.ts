...
describe("LocalizedStringSchema", () => {
  ...
  it("accepts EN + any subset of other locales", () => {
    ...
    expect(
      LocalizedStringSchema.safeParse({ en: "Hello", de: "Hallo" }).success,
    ).toBe(true);
  });

  it("rejects unknown keys (strict mode)", () => {
    expect(
      LocalizedStringSchema.safeParse({ en: "Hello", fr: "Bonjour" }).success,
    ).toBe(false);
  });

  it("rejects empty non-source locales", () => {
    expect(
      LocalizedStringSchema.safeParse({ en: "Hello", ru: "" }).success,
    ).toBe(false);
  });
